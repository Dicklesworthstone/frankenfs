...
| btrfs multi-device RAID | `fs/btrfs/volumes.c` | 🟡 | `BtrfsDeviceSet` provides segmented stripe/chunk reads and exact-length mirror fallback. RAID5/6 helpers read the owning data stripe without parity reconstruction. Unit and harness tests cover these helpers, not mounted device identity or dispatch. `OpenFs` still uses Single/Dup address translation; V1 multi-device mounted reads and kernel-generated-image evidence remain open (`bd-hk5w3`, `bd-mjxxk`). Multi-device writes remain deferred. |
...
| btrfs device tree discovery | `fs/btrfs/volumes.c`, `fs/btrfs/ioctl.c` | 🟡 | Complete device enumeration remains unwired: DEV_ITEM records belong in CHUNK_TREE, while DEV_TREE holds device extents. `walk_device_tree()` traversal and synthetic DEV_ITEM parser coverage do not prove a complete inventory. `BTRFS_IOC_DEV_INFO` verifies backing superblock identity, then reports accounting from the live writable chunk tree or an exact lookup in the committed read-only chunk tree. Missing, malformed, mismatched, or partial records are refused. Core coverage includes noncontiguous and maximum device IDs, allocation before writeback, full commit/reopen accounting, and an external `btrfs check` assertion on the committed image. `btrfs_openfs_ioctl_dev_info_payload_contract` compares read-only and newly writable replies with independent raw image bytes. Lookup requires an exact device ID; only UUID is optional. Single-device `FS_INFO.max_id` uses verified identity; incomplete multi-device inventories return an explicit unsupported error. Superblock-publication synchronization, complete multi-device enumeration, and mounted routing remain open (`bd-hk5w3`). |
