...
| btrfs multi-device RAID | `fs/btrfs/volumes.c` | 🟡 | `BtrfsDeviceSet` provides segmented stripe/chunk reads and exact-length mirror fallback with caller `Cx`, typed runtime errors, cancellation before/after I/O and non-replacing device registration. Cancellation stops mirror retries. RAID5/6 helpers read the owning data stripe without parity reconstruction. Unit and harness tests cover these helpers, including actual file-backed reads with a synthetic RAID layout, not mounted device identity or dispatch. `OpenFs` still uses Single/Dup address translation; V1 multi-device mounted reads, checksum-aware mirror selection and kernel-generated-image evidence remain open (`bd-hk5w3`, `bd-mjxxk`). Multi-device writes remain deferred. |
...
| btrfs device tree discovery | `fs/btrfs/volumes.c`, `fs/btrfs/ioctl.c` | 🟡 | `FS_INFO` and `DEV_INFO` now enumerate all DEV_ITEM records from the live writable or checksum-verified committed CHUNK_TREE; DEV_TREE holds device extents instead. Validation checks record keys/IDs, UUID uniqueness, filesystem membership, device count and backing identity. `FS_INFO.max_id` covers sparse IDs and unused devices; `DEV_INFO` accepts any inventory ID with optional exact UUID and current per-device accounting. Core tests cover maximum IDs, unused devices, corrupt/partial inventories, live allocation and full commit/reopen with independent `btrfs check`. A two-device tool-generated image test compares both devices against independently read raw fields; single-device image and mounted ioctl regressions remain covered. Additional backing-file validation/attachment and multi-device read routing are still unwired. Seed-device inventories with different filesystem UUIDs, count-mismatch repair and superblock-publication synchronization remain gaps (`bd-hk5w3`); no multi-device I/O parity is claimed. |
