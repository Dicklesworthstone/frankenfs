...
| btrfs multi-device RAID | `fs/btrfs/volumes.c` | 🟡 | `OpenOptions::btrfs_device_paths` / repeatable CLI `--btrfs-device` connects validated backings to bootstrap, metadata and file-data reads. Clean multi-device images use device-set routing even without extra paths. Each lone RAID1 survivor passes core/FUSE ordinary, zstd and NODATASUM reads. Every committed chunk requires coverage: RAID1/C3/C4 need one copy; RAID10 needs a survivor in every validated mirror group. Four-device kernel RAID10 tests cover all 14 nonempty proper subsets, with core/FUSE ordinary-data reads for supported subsets and refusal when a group is lost. Three-device C3/four-device C4 kernel images cover all 6/14 proper nonempty subsets through ordinary full-file/unaligned core reads and cold FUSE reads; profile geometry is validated before admission. C3/C4 corruption/compression evidence remains pending. Mixed mirrored-metadata/RAID0-data images reject missing data devices. No implicit sibling discovery. Healthy RAID0/RAID1/RAID10/C3/C4 reads pass with each primary; foreign/duplicate attachments and cancelled opens are rejected. Metadata and checksummed data retry invalid mirrors; RAID1 chunk/root/fs-tree and ordinary/zstd corruption tests cover recovery, two-bad-copy refusal, unaligned reads and opt-out behavior. Reads use caller `Cx`; cancellation stops retries. Multi-device mounts require clean tree logs and no MVCC WAL. Dirty-image recovery and the remaining profile/degraded matrix remain open (`bd-hk5w3`, `bd-mjxxk`). RAID5/6 helpers have no parity reconstruction. Multi-device writes remain deferred and refused. |
