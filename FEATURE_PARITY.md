...
| btrfs multi-device RAID | `fs/btrfs/volumes.c` | 🟡 | `OpenOptions::btrfs_device_paths` / repeatable CLI `--btrfs-device` connects validated backings to bootstrap, metadata and file-data reads. Kernel-written RAID0 and RAID1 payloads pass core and FUSE reads with either primary device; foreign/duplicate attachments and cancelled opens are rejected. Metadata retries checksum-invalid or structurally invalid copies before caching; RAID1 chunk/root/fs-tree corruption tests recover through FUSE with one bad copy and refuse two bad copies. Checksummed file reads validate the exact sector bytes copied or decompressed; RAID1 ordinary/zstd data corruption recovers through FUSE, with unaligned-read, two-bad-copy refusal, opt-out and NODATASUM countercases. Reads use caller `Cx`; cancellation stops retries. Attached mounts require clean tree logs and no MVCC WAL. Dirty-image recovery, degraded reads and the full profile matrix remain open (`bd-hk5w3`, `bd-mjxxk`). RAID5/6 helpers have no parity reconstruction. Multi-device writes remain deferred and refused. |
