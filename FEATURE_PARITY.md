...
| btrfs multi-device RAID | `fs/btrfs/volumes.c` | 🟡 | `OpenOptions::btrfs_device_paths` / repeatable CLI `--btrfs-device` connects validated backings to bootstrap, metadata and file-data reads. Clean multi-device images use device-set routing even without extra paths. Each lone RAID1 survivor passes core/FUSE ordinary, zstd and NODATASUM reads; all committed chunks must have supported device coverage. Missing stripes are admitted only for RAID1 with a surviving copy; mixed mirrored-metadata/RAID0-data images reject missing data devices. No implicit sibling discovery. Healthy RAID0/RAID1 reads pass with either primary; foreign/duplicate attachments and cancelled opens are rejected. Metadata and checksummed data retry invalid mirrors; chunk/root/fs-tree and ordinary/zstd corruption tests cover recovery, two-bad-copy refusal, unaligned reads and opt-out behavior. Reads use caller `Cx`; cancellation stops retries. Multi-device mounts require clean tree logs and no MVCC WAL. Dirty-image recovery and the remaining profile/degraded matrix remain open (`bd-hk5w3`, `bd-mjxxk`). RAID5/6 helpers have no parity reconstruction. Multi-device writes remain deferred and refused. |
