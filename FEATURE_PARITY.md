...
| btrfs device tree discovery | `fs/btrfs/volumes.c`, `fs/btrfs/ioctl.c` | 🟡 | Complete device enumeration remains unwired: DEV_ITEM records belong in CHUNK_TREE, while DEV_TREE holds device extents. `walk_device_tree()` traversal and synthetic DEV_ITEM parser coverage do not prove a complete inventory. `BTRFS_IOC_DEV_INFO` verifies the backing superblock device identity; writable mounts report current accounting from the authoritative in-memory chunk tree after validating its identity. Core tests exercise real chunk allocation on a tool-generated image and reject malformed, missing, or partial live records. `btrfs_openfs_ioctl_dev_info_payload_contract` compares both read-only and newly writable replies with independent raw image bytes. Lookup requires an exact device ID; only the UUID may be unspecified. Single-device `FS_INFO.max_id` uses the verified identity, including noncontiguous IDs; incomplete multi-device inventories return an explicit unsupported error. Read-only post-growth accounting still comes from the superblock. Mounted multi-device enumeration and routing remain open (`bd-hk5w3`). |
