...
| btrfs device tree discovery | `fs/btrfs/volumes.c`, `fs/btrfs/ioctl.c` | 🟡 | Complete device enumeration remains unwired: DEV_ITEM records belong in CHUNK_TREE, while DEV_TREE holds device extents. `walk_device_tree()` traversal and synthetic DEV_ITEM parser coverage do not prove a complete inventory. `BTRFS_IOC_DEV_INFO` uses the backing superblock's checksum-verified device item, including its actual ID, UUID, and on-disk capacity/accounting. `btrfs_openfs_ioctl_dev_info_payload_contract` compares replies with independent raw bytes from a mkfs-generated image. Core tests cover noncontiguous IDs and corrupt identity/accounting. Lookup requires an exact device ID; only the UUID may be unspecified, matching Linux v6.19. Single-device `FS_INFO.max_id` comes from the same verified identity and can discover that device through `DEV_INFO`; incomplete multi-device inventories return an explicit unsupported error. Mounted multi-device enumeration and live allocation accounting remain outside this evidence (`bd-hk5w3`). |
