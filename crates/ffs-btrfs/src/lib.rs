...
mod tests {
    ...
    fn reconciled_block_group_accounting_makes_a_preexisting_extent_freeable_bd_ftev0() {
        ...
        // Negative case: in that unreconciled state the free must still fail
        // closed. An implementation that "fixed" bd-ftev0 by softening the
        // underflow guard instead of correcting the tally would pass every
        // assertion below and fail this one.
        assert_eq!(
            alloc
                .free_extent(bg_start, 4096, false)
                .expect_err("unreconciled accounting must fail closed"),
            BtrfsMutationError::BrokenInvariant("block group used bytes underflow")
        );
