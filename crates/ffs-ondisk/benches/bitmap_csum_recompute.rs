...
/// Raw crc32c over a whole group bitmap vs over just the changed region.
fn bench_bitmap_csum(c: &mut Criterion) {
...
fn bench_bitmap_csum_value(c: &mut Criterion) {
...
/// Split into one function per group (bd-3ao0l): each `BenchmarkGroup` then ends
/// at its own `finish()` rather than living to the end of `bench`, and the
/// mid-function `use` moves to the top where it is in scope from the start.
fn bench(c: &mut Criterion) {
    bench_bitmap_csum(c);
    bench_bitmap_csum_value(c);
}
