...
use cmd_dump::dump_cmd;
#[cfg(test)]
use cmd_dump::{
    build_dump_dir_output, build_dump_group_output, build_dump_inode_output, bytes_to_hex_dump,
};
use cmd_inspect::inspect;
#[cfg(test)]
use cmd_inspect::inspect_ext4_output;
use cmd_repair::{
...
#[cfg(test)]
use ffs_btrfs::{BTRFS_FS_TREE_OBJECTID, BTRFS_ITEM_INODE_ITEM, BTRFS_ITEM_ROOT_ITEM};
...
use ffs_ondisk::{
    BtrfsSuperblock, Ext4ExtentHeader, Ext4ExtentIndex, Ext4GroupDesc, Ext4ImageReader, Ext4Inode,
    Ext4Superblock, ext4::Ext4QuotaInodes,
};
...
#[cfg(test)]
use ffs_types::{BTRFS_SUPER_INFO_OFFSET, BTRFS_SUPER_INFO_SIZE};
use ffs_types::{BlockNumber, ByteOffset, GroupNumber, InodeNumber, MountMode};
use mount_console::{
...
enum Command {
    ...
    Parity {
        ...
        #[arg(long)]
        json: bool,
        /// Execute ext4-journal, ext4-reference, btrfs-reference, or the parity-honesty self-check.
        #[arg(long)]
        verify: Vec<String>,
...
fn log_mount_shutdown_metrics(
    ...
    info!(
        ...
        requests_err = metrics.requests_err,
        bytes_read = metrics.bytes_read,
        metadata_requests = metrics.metadata_requests,
...
fn mount_cmd(image_path: &Path, mountpoint: &Path, options: &MountCmdOptions) -> Result<()> {
    if env_bool("FFS_MOUNT_BENCH_EVIDENCE", false)? {
        ...
        eprintln!(
            ...
            // bd-3d2c0: both were implemented in the transport and readable from
            // the environment while ABSENT from this line, so neither could be
            // A/B'd admissibly. A knob the daemon does not self-report is
            // unattestable (bd-087wt).
            ffs_fuse::receive_spin_adaptive(),
            ffs_fuse::spin_pause(),
            ...
            // bd-mf9z9: the cache is useful only if its byte budget is attested
            // by the daemon that owns it; report the resolved value, not merely
            // the environment the comparator requested.
            ffs_core::ext4_file_data_extent_cache_bytes(),
            ...
            // 2026-08-27: reported so the flush-path realloc chain can be A/B'd
            // from ONE ELF on a mutating row, same rule as every knob above.
            ffs_mvcc::flush_run_reserve_enabled(),
            // 2026-08-27: the borrow path removes the flush's per-block clone; reported
            // so it can be A/B'd from ONE ELF like every knob above.
            ffs_mvcc::flush_borrow_enabled(),
            ...
            // 2026-08-27: the gather-list flush; reported so it can be A/B'd from
            // ONE ELF like every knob here.
            ffs_mvcc::flush_vectored_enabled(),
            ...
            // 2026-08-27 (bd-087wt): this knob was NOT on the line, so a
            // FFS_FUSE_PARENT_INVAL A/B could only be attested from the crossing
...
mod tests {
    ...
    fn sharded_create_mount_defaults_on_after_the_delete_side_lands_bd_y2t0r() {
        ...
        // Still explicitly opt-in-able, so the A/B and the perf work stay available.
