...
fn run_shared_btrfs_rw_scenario(scenario: &SharedBtrfsScenario) {
...

    {
        ...
        if !attached.status.success() {
            require_fuse_or_skip("kernel loop attach unavailable");
            return;
        }
...
fn assert_btrfs_mirror_survivors(cx: &Cx, images: &[PathBuf], payload: &[u8], profile: &str) {
    ...
    .unwrap();
    let chunks = full.btrfs_context().unwrap().chunks.clone();
    drop(full);
    ...
    for mask in 1_u32..full_mask {
        ...
        let readable = chunks.iter().all(|chunk| {
            ...
            pairs.iter().all(|pair| {
                pair.iter()
                    .any(|stripe| selected.iter().any(|index| ids[*index] == stripe.devid))
            })
        });
        let options = OpenOptions {
            btrfs_device_paths: selected[1..]
                .iter()
        ...
        if readable {
            ...
            assert!(
                opened.is_err(),
                "RAID10 subset {mask:04b} loses an entire mirror pair"
            );
            refused += 1;
        }
    }
