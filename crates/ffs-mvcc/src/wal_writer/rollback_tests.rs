...
fn assert_sealed(writer: &mut WalWriter, path: &Path) {
    ...
    for error in [
        ...
        writer.append_commit(&commit(10, 10, 1)).expect_err("sealed append"),
        writer.append_commits_coalesced(&[]).expect_err("sealed empty batch"),
        writer
            .append_commits_coalesced(&[commit(10, 10, 1), commit(11, 11, 1)])
            .expect_err("sealed coalesced append"),
        writer.flush().expect_err("sealed flush must not acknowledge zero pending"),
    ] {
    ...
    assert_eq!(std::fs::read(path).unwrap(), bytes, "sealed calls must not touch storage");
}
...
fn partial_append_restores_the_exact_prefix_before_a_smaller_retry() {
    ...
    let error = writer.append_commit(&commit(2, 2, 4096)).expect_err("partial write");
    ...
    writer.ensure_ready().expect("durable rollback permits retry");
...
fn complete_but_unacknowledged_record_is_removed_on_append_error() {
    ...
    writer.append_commit(&commit(1, 1, 128)).expect_err("error after full record");
    assert_eq!(std::fs::read(&path).unwrap(), prefix);
    assert_eq!(writer.last_commit_seq(), 0);
    assert_eq!(writer.pending_sync_count(), 0);
    writer.ensure_ready().expect("rollback includes mandatory sync in Manual mode");
    drop(writer);
...
fn coalesced_partial_append_never_publishes_its_complete_first_record() {
    ...
    writer.append_commits_coalesced(&batch).expect_err("partially written batch");
    assert_eq!(std::fs::read(&path).unwrap(), prefix);
    assert_eq!(writer.last_commit_seq(), 1);
    assert_eq!(writer.pending_sync_count(), 0);
    writer.fail_append_after = None;
    let retry = commit(2, 4, 1);
    writer.append_commit(&retry).expect("reuse failed batch sequence");
    drop(writer);
    assert_eq!(replay_file(&path), vec![first, retry]);
}
...
fn failed_truncate_seals_writer_with_a_replayable_unacknowledged_record() {
    ...
    let error = writer.append_commit(&uncertain).expect_err("rollback cannot truncate");
    assert!(matches!(error, WalWriteError::RecoveryRequired { .. }));
...
fn rollback_sync_failure_is_not_mistaken_for_a_completed_rollback() {
    ...
    let error = writer.append_commit(&commit(1, 1, 64)).expect_err("rollback not durable");
    assert!(matches!(error, WalWriteError::RecoveryRequired { .. }));
...
fn sync_failure_rolls_back_only_the_new_record_and_preserves_pending_prefix() {
    ...
    let error = writer.append_commit(&commit(3, 3, 1024)).expect_err("batch sync fails");
...
fn coalesced_sync_failure_seals_if_the_batch_cannot_be_removed() {
    ...
    let error = writer.append_commits_coalesced(&batch).expect_err("batch rollback failure");
    assert!(matches!(error, WalWriteError::RecoveryRequired { .. }));
    assert_eq!(writer.last_commit_seq(), 0);
...
fn verification_read_error_uses_the_same_durable_rollback() {
    ...
    let mut writer = WalWriter::new(file, offset, WalWriterConfig {
        verify_writes: true,
        ..WalWriterConfig::default()
    });
    let error = writer.append_commit(&commit(1, 1, 512)).expect_err("readback on write-only fd");
    assert!(matches!(error, WalWriteError::AppendIo { .. }));
    assert_eq!(std::fs::read(&path).unwrap(), prefix);
    ...
    writer.append_commit(&retry).expect("retry after verified rollback");
    drop(writer);
...
fn coalesced_verification_failure_cannot_hide_failed_rollback() {
    ...
    let error = writer.append_commits_coalesced(&batch).expect_err("readback and rollback failure");
    assert!(matches!(error, WalWriteError::RecoveryRequired { .. }));
    assert_eq!(replay_file(&path), batch);
    assert_sealed(&mut writer, &path);
}
...
fn append_offset_overflow_is_rejected_before_io_or_rollback() {
    ...
    writer.fail_rollback_truncate = true;
    let error = writer.append_commit(&commit(1, 1, 1)).expect_err("overflowing offset");
    assert!(matches!(error, WalWriteError::FormatViolation { .. }));
    assert_eq!(std::fs::read(&path).unwrap(), prefix);
    writer.ensure_ready().expect("preflight error must not attempt rollback or poison");
    writer.write_pos = actual_offset;
    writer.fail_rollback_truncate = false;
    writer.append_commit(&commit(1, 2, 1)).expect("healthy writer remains usable");
}

#[test]
fn recovery_required_maps_to_io_error_and_is_not_retryable() {
    let error = WalWriteError::RecoveryRequired { detail: "uncertain tail".to_owned() };
    assert!(error.is_fatal());
    assert!(!error.is_retryable());
