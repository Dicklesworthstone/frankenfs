...
K1=/home/ubuntu/warm4-k1
K2=/home/ubuntu/warm4-k2
FA=/home/ubuntu/warm4-fa
FB=/home/ubuntu/warm4-fb
...
cp "$W/wimg4-base.ext4" "$W/wimg4-fa.ext4"
cp "$W/wimg4-base.ext4" "$W/wimg4-fb.ext4"

sudo -n mount -o loop,ro "$W/wimg4-k1.ext4" "$K1"
sudo -n mount -o loop,ro "$W/wimg4-k2.ext4" "$K2"
...
start_fuse() {  # $1=cpus $2=mnt $3=img $4=logsuffix $5=extra env string
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=1 FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount "$3" "$2" >> "$W/w4fuse-$TAG-$4.log" 2>&1 &
  echo $!
}

APID=$(start_fuse "$FA_CPUS" "$FA" "$W/wimg4-fa.ext4" "a" "$FA_ENV")
BPID=0
if [ -n "$FB_LABEL" ]; then
  BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/wimg4-fb.ext4" "b" "$FB_ENV")
fi
...
wait_mount "$FA" "$APID" "$W/w4fuse-$TAG-a.log"
ARMS=("k1=$K1" "k2=$K2" "$FA_LABEL=$FA")
if [ -n "$FB_LABEL" ]; then
  wait_mount "$FB" "$BPID" "$W/w4fuse-$TAG-b.log"
  ARMS+=("$FB_LABEL=$FB")
fi

echo "== $FA_LABEL pid $APID cpus $FA_CPUS env '$FA_ENV'"
grep -h "mount_candidate_knobs" "$W/w4fuse-$TAG-a.log" | tail -1
if [ -n "$FB_LABEL" ]; then
  echo "== $FB_LABEL pid $BPID cpus $FB_CPUS env '$FB_ENV'"
  grep -h "mount_candidate_knobs" "$W/w4fuse-$TAG-b.log" | tail -1
fi
...
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/w4fuse-$TAG-a.log" | tail -3
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/w4fuse-$TAG-b.log" | tail -3
fi
