...
sudo -n mount -o loop "$W/bimg-k1.ext4" "$K1"
sudo -n mount -o loop "$W/bimg-k2.ext4" "$K2"
...
APID=$(start_fuse "$FA_CPUS" "$FA" "$W/bimg-fa.ext4" "a" "$FA_ENV")
BPID=0
[ -n "$FB_LABEL" ] && BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/bimg-fb.ext4" "b" "$FB_ENV")
...
"$W/bulkwrite_ab" "$ROUNDS" "$CHUNKS" "$CPU" "$APID" "${ARMS[@]}" | tee "$W/bulk-$TAG.csv"
