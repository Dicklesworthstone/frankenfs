...
K1=/home/ubuntu/bstorm-k1
K2=/home/ubuntu/bstorm-k2
FA=/home/ubuntu/bstorm-fa
FB=/home/ubuntu/bstorm-fb
...
for n in k1 k2 fa fb; do python3 "$W/mkcopy.py" "$W/bimgb-base.btrfs" "$W/bimgb-$n.btrfs"; done
sync
...
DK1=$(attach "$W/bimgb-k1.btrfs"); LOOPS="$LOOPS $DK1"
DK2=$(attach "$W/bimgb-k2.btrfs"); LOOPS="$LOOPS $DK2"
DFA=$(attach "$W/bimgb-fa.btrfs"); LOOPS="$LOOPS $DFA"
DFB=$(attach "$W/bimgb-fb.btrfs"); LOOPS="$LOOPS $DFB"
...
start_fuse() {  # $1=cpus $2=mnt $3=img $4=suffix $5=env
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=${BENCH_EV:-1} FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount --rw "$3" "$2" >> "$W/bsfuse2-$TAG-$4.log" 2>&1 &
  echo $!
}
...
wait_mount "$FA" "$APID" "$W/bsfuse2-$TAG-a.log"
ARMS=("k1=$K1" "k2=$K2" "$FA_LABEL=$FA")
if [ -n "$FB_LABEL" ]; then
  wait_mount "$FB" "$BPID" "$W/bsfuse2-$TAG-b.log"
  ARMS+=("$FB_LABEL=$FB")
fi

echo "== $FA_LABEL pid $APID cpus $FA_CPUS env '$FA_ENV'"
grep -h "mount_candidate_knobs" "$W/bsfuse2-$TAG-a.log" | tail -1 || true
if [ -n "$FB_LABEL" ]; then
  echo "== $FB_LABEL pid $BPID cpus $FB_CPUS env '$FB_ENV'"
  grep -h "mount_candidate_knobs" "$W/bsfuse2-$TAG-b.log" | tail -1 || true
fi
...
"$W/storm_ab" "$ROUNDS" "$OPS" "$CPU" "$APID" "${ARMS[@]}" | tee "$W/bstormdio-$TAG.csv"

echo "== unmount + census"
fusermount3 -u "$FA"; wait "$APID" 2>/dev/null || true
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts" "$W/bsfuse2-$TAG-a.log" | tail -2 || true
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts" "$W/bsfuse2-$TAG-b.log" | tail -2 || true
fi
