...
sudo -n mount -o loop "$W/simg-k1.ext4" "$K1"
sudo -n mount -o loop "$W/simg-k2.ext4" "$K2"
...
APID=$(start_fuse "$FA_CPUS" "$FA" "$W/simg-fa.ext4" "a" "$FA_ENV")
BPID=0
[ -n "$FB_LABEL" ] && BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/simg-fb.ext4" "b" "$FB_ENV")
...
"$W/storm_ab" "$ROUNDS" "$OPS" "$CPU" "$APID" "${ARMS[@]}" | tee "$W/storm-$TAG.csv"
