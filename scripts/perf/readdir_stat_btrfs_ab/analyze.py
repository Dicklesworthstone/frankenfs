...
for a in names:
    ...
    stt = [int(x["stat_ns"]) for x in arms[a].values()]
    rdd = [int(x["readdir_ns"]) for x in arms[a].values()]
    tk = [int(x["daemon_ticks"]) for x in arms[a].values()]
    print(f"  {a:12s} n={len(tot):3d} total_med={st.median(tot)/1e6:9.3f}ms "
          f"readdir_med={st.median(rdd)/1e6:8.3f}ms stat_med={st.median(stt)/1e6:9.3f}ms "
