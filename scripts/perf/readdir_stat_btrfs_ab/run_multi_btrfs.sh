...
K1=/home/ubuntu/rdstatb-k1
K2=/home/ubuntu/rdstatb-k2
FA=/home/ubuntu/rdstatb-fa
FB=/home/ubuntu/rdstatb-fb
...
cp "$W/bimgr-base.btrfs" "$W/bimgr-fa.btrfs"
cp "$W/bimgr-base.btrfs" "$W/bimgr-fb.btrfs"

sudo -n mount -o loop,ro "$W/bimgr-k1.btrfs" "$K1"
sudo -n mount -o loop,ro "$W/bimgr-k2.btrfs" "$K2"
...
start_fuse() {  # $1=cpus $2=mnt $3=img $4=logsuffix $5=extra env string
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=1 FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount "$3" "$2" >> "$W/bfuse-$TAG-$4.log" 2>&1 &
  echo $!
}

APID=$(start_fuse "$FA_CPUS" "$FA" "$W/bimgr-fa.btrfs" "a" "$FA_ENV")
BPID=0
if [ -n "$FB_LABEL" ]; then
  BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/bimgr-fb.btrfs" "b" "$FB_ENV")
fi
...
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/bfuse-$TAG-a.log" | tail -3
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/bfuse-$TAG-b.log" | tail -3
fi
