#!/bin/bash
# Interleaved btrfs parallel-metadata-write with FOUR arms of the SAME shape:
# two live kernel btrfs mounts (the A/A null) and TWO FrankenFS btrfs mounts from
# ONE ELF. The six-arm run_pmeta_btrfs_dio.sh cannot price a btrfs-only knob --
# its two FUSE arms are different FILESYSTEMS (pimg-fa.ext4 and pimgb-fb.btrfs),
# so an A/B across them measures ext4-vs-btrfs (2026-08-27, rejected measurement).
# This rig exists so a btrfs write knob has two btrfs arms to differ on.
...
K1=/home/ubuntu/bpmeta-k1
K2=/home/ubuntu/bpmeta-k2
FA=/home/ubuntu/bpmeta-fa
FB=/home/ubuntu/bpmeta-fb
...
for n in k1 k2 fa fb; do python3 "$W/mkcopy.py" "$W/pmb-base.btrfs" "$W/pmb-$n.btrfs"; done
sync
...
DK1=$(attach "$W/pmb-k1.btrfs"); LOOPS="$LOOPS $DK1"
DK2=$(attach "$W/pmb-k2.btrfs"); LOOPS="$LOOPS $DK2"
DFA=$(attach "$W/pmb-fa.btrfs"); LOOPS="$LOOPS $DFA"
DFB=$(attach "$W/pmb-fb.btrfs"); LOOPS="$LOOPS $DFB"
...
start_fuse() {  # $1=cpus $2=mnt $3=img $4=suffix $5=env
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=${BENCH_EV:-1} FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount --rw "$3" "$2" >> "$W/bpm2fuse-$TAG-$4.log" 2>&1 &
  echo $!
}
...
wait_mount "$FA" "$APID" "$W/bpm2fuse-$TAG-a.log"
if [ -n "$FB_LABEL" ]; then
  wait_mount "$FB" "$BPID" "$W/bpm2fuse-$TAG-b.log"
  ARMS+=("$FB_LABEL=$FB")
fi

echo "== $FA_LABEL pid $APID cpus $FA_CPUS env '$FA_ENV'"
grep -h "mount_candidate_knobs" "$W/bpm2fuse-$TAG-a.log" | tail -1 || true
if [ -n "$FB_LABEL" ]; then
  echo "== $FB_LABEL pid $BPID cpus $FB_CPUS env '$FB_ENV'"
  grep -h "mount_candidate_knobs" "$W/bpm2fuse-$TAG-b.log" | tail -1 || true
fi
echo "== client cpu $CPUBASE, $OPS create+delete pairs per batch"

S() { echo "/sys/block/$(basename "$1")/stat"; }
ARMS=("k1=$K1" "k2=$K2" "$FA_LABEL=$FA")
[ -n "$FB_LABEL" ] && ARMS+=("$FB_LABEL=$FB")
"$W/pmeta_ab" "$ROUNDS" "$OPS" "$THREADS" "$CPUBASE" "$APID" "${ARMS[@]}" | tee "$W/bpmetadio-$TAG.csv"

echo "== unmount + census"
fusermount3 -u "$FA"; wait "$APID" 2>/dev/null || true
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts" "$W/bpm2fuse-$TAG-a.log" | tail -2 || true
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts" "$W/bpm2fuse-$TAG-b.log" | tail -2 || true
fi
