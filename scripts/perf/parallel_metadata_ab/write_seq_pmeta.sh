...
cleanup
mkdir -p "$FM"
python3 "$W/mkcopy.py" "$W/pimg-base.ext4" "$W/pimg-ws.ext4"
DEV=$(sudo -n losetup --find --show --direct-io=on "$W/pimg-ws.ext4")
LOOPS="$DEV"
sudo -n chown "$(id -u)" "$DEV"

# shellcheck disable=SC2086
env RUST_LOG=warn $FENV taskset -c 8-15 "$ELF" mount --rw "$DEV" "$FM" \
...
SPID=$!
sleep 1
"$W/pmeta_ab" 1 "$OPS" 8 8 0 "f=$FM" >/dev/null 2>&1
sleep 1
