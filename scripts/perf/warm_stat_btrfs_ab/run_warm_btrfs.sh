...
K1=/home/ubuntu/warmb-k1
K2=/home/ubuntu/warmb-k2
FA=/home/ubuntu/warmb-fa
FB=/home/ubuntu/warmb-fb
...
cp "$W/wimgb-base.btrfs" "$W/wimgb-fa.btrfs"
cp "$W/wimgb-base.btrfs" "$W/wimgb-fb.btrfs"

sudo -n mount -o loop,ro "$W/wimgb-k1.btrfs" "$K1"
sudo -n mount -o loop,ro "$W/wimgb-k2.btrfs" "$K2"
...
start_fuse() {  # $1=cpus $2=mnt $3=img $4=logsuffix $5=extra env string
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=1 FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount "$3" "$2" >> "$W/wfuse-$TAG-$4.log" 2>&1 &
  echo $!
}

APID=$(start_fuse "$FA_CPUS" "$FA" "$W/wimgb-fa.btrfs" "a" "$FA_ENV")
BPID=0
if [ -n "$FB_LABEL" ]; then
  BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/wimgb-fb.btrfs" "b" "$FB_ENV")
fi
...
wait_mount "$FA" "$APID" "$W/wfuse-$TAG-a.log"
ARMS=("k1=$K1" "k2=$K2" "$FA_LABEL=$FA")
if [ -n "$FB_LABEL" ]; then
  wait_mount "$FB" "$BPID" "$W/wfuse-$TAG-b.log"
  ARMS+=("$FB_LABEL=$FB")
fi

echo "== $FA_LABEL pid $APID cpus $FA_CPUS env '$FA_ENV'"
grep -h "mount_candidate_knobs" "$W/wfuse-$TAG-a.log" | tail -1
if [ -n "$FB_LABEL" ]; then
  echo "== $FB_LABEL pid $BPID cpus $FB_CPUS env '$FB_ENV'"
  grep -h "mount_candidate_knobs" "$W/wfuse-$TAG-b.log" | tail -1
fi
echo "== client cpu $CPUBASE, $OPS stats/batch (single thread)"

"$W/warmstat_ab" "$ROUNDS" "$OPS" "$CPUBASE" "$APID" "${ARMS[@]}" | tee "$W/warm-$TAG.csv"

echo "== unmount + census"
fusermount3 -u "$FA"; wait "$APID" 2>/dev/null || true
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/wfuse-$TAG-a.log" | tail -3
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts\|getattr_split" "$W/wfuse-$TAG-b.log" | tail -3
fi
