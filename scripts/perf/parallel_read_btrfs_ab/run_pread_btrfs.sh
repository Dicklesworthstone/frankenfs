#!/bin/bash
# Interleaved parallel-read-8t on BTRFS (bd-6kpp4 item 3): two LIVE kernel ext4 ro loop mounts (the A/A null) and
...
K1=/home/ubuntu/preadb-k1
K2=/home/ubuntu/preadb-k2
FA=/home/ubuntu/preadb-fa
FB=/home/ubuntu/preadb-fb
LOOPS=""
...
for n in k1 k2 fa fb; do cp "$W/rimgb-base.btrfs" "$W/rimgb-$n.btrfs"; done
sync

sudo -n mount -o loop,ro "$W/rimgb-k1.btrfs" "$K1"
sudo -n mount -o loop,ro "$W/rimgb-k2.btrfs" "$K2"
...
# $7 carries per-arm CLI ARGS: bd-6kpp4's knob is --btrfs-verify-data-on-read, a
# clap flag rather than an env var, so an env-only hook cannot A/B it.
start_fuse() {  # $1=cpus $2=mnt $3=img $4=suffix $5=env $6=useloop $7=args
  local img="$3"
  if [ "$6" = "1" ]; then
    img=$(attach_loop "$3")
    LOOPS="$LOOPS $img"
  ...
  env FFS_MOUNT_BENCH_EVIDENCE=1 FFS_OP_COUNTS=1 RUST_LOG=warn $5 \
    taskset -c "$1" "$ELF" mount $7 "$img" "$2" >> "$W/rbfuse-$TAG-$4.log" 2>&1 &
  echo $!
}
APID=$(start_fuse "$FA_CPUS" "$FA" "$W/rimgb-fa.btrfs" "a" "$FA_ENV" "$FA_LOOP" "$FA_ARGS")
BPID=0
[ -n "$FB_LABEL" ] && BPID=$(start_fuse "$FB_CPUS" "$FB" "$W/rimgb-fb.btrfs" "b" "$FB_ENV" "$FB_LOOP" "$FB_ARGS")
...
wait_mount "$FA" "$APID" "$W/rbfuse-$TAG-a.log"
ARMS=("k1=$K1" "k2=$K2" "$FA_LABEL=$FA")
if [ -n "$FB_LABEL" ]; then
  wait_mount "$FB" "$BPID" "$W/rbfuse-$TAG-b.log"
  ARMS+=("$FB_LABEL=$FB")
fi
...
echo "--- $FA_LABEL"; grep -h "crossings_total\|op_counts" "$W/rbfuse-$TAG-a.log" | tail -2 || true
if [ -n "$FB_LABEL" ]; then
  fusermount3 -u "$FB"; wait "$BPID" 2>/dev/null || true
  echo "--- $FB_LABEL"; grep -h "crossings_total\|op_counts" "$W/rbfuse-$TAG-b.log" | tail -2 || true
fi
