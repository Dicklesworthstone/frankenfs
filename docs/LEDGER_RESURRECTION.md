...
|---|---|---|
| `NEGATIVE_EVIDENCE.md:332` — scrub zero-check word→SIMD 64-byte OR-fold | Manual 4×u64 OR-fold vs a 64-byte/eight-`u64` reduction; rejected because *"the word-wise `all()` already auto-vectorizes"* and cycles were flat despite 7–13% fewer instructions | **SUPERSEDED after live-caller audit (2026-07-26), not rank 1.** The SSE2 measurement still cannot decide the AVX2 code-generation question, but the **12.89% production attribution is stale**: correctness commit `b2db6d0e` removed blind `ZeroCheckValidator` from Btrfs and ext4 whole-image scrub on 2026-07-09 because it treated legitimate zeroed free space as corruption. Current whole-image scrub does not call `is_all_zero`; its only non-test caller is the scoped erasure-symbol reader in `ffs-repair::storage`. A source-neutral v3 microbenchmark would therefore measure a dead scrub caller and cannot justify changing production. Retry only after a fresh current-production scoped validator or erasure-recovery profile attributes at least 5% self-time to `is_all_zero`, with the full ELF/A/A/median-CI contract. Canonical audit correction: `bd-scrub-zero-scan-v3-y7pbf` in `docs/NEGATIVE_EVIDENCE.md`. |
| `NEGATIVE_EVIDENCE.md:209` — `last_nonzero_len` reverse zero-scan | Refuted by reading it: *"already OR-reduces 32 bytes (4 u64s) per iter — optimal"* | **VOID-ISA rationale confirmed, but retry NOT ADMITTED by live profile (2026-07-26).** "Optimal" was an SSE2 codegen judgement, so AVX2 could still change the primitive result. However, the current delete profile attributes only **1.08% self** to the entire enclosing `ext4_chksum_skip_zero_tail` function and does not rank `last_nonzero_len` independently. Even eliminating the whole function has a **1.0109x** Amdahl ceiling. Do not manufacture a source-neutral SIMD ratio or re-derive the closed checksum frontier. Retry only after a fresh production profile puts `last_nonzero_len` at least 5% self, or puts the enclosing helper at least 5% with a counted scan stage consuming at least 25% of its cycles; then use the full executing-ELF/A/A/median-CI contract. Canonical audit: `bd-void-isa-last-nonzero-audit-c8imz` in `docs/NEGATIVE_EVIDENCE.md`. |
