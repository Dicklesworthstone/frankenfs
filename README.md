...
|---|---|---|
| **Block-level MVCC** | Version chains per block, snapshot isolation, 4 executable same-block merge mechanisms (`AppendOnly`, `RangeOverlay`, `BitmapOr`, `BitmapDelta`) exposed through `MergeProof` labels, the 5-outcome `MergeProofMechanism` enum (those four plus `NoSameBlockMerge`), and three `ConflictPolicy` modes (`Strict` / `SafeMerge` / `Adaptive`) selected by an expected-loss decision model | Selected ext4 write paths derive range and disjoint-block proofs and stage them through MVCC. The historical 120-writer benchmark reported 9.5× lower expected loss for SafeMerge than Strict; it does not establish that benefit for mounted workloads. JBD2 attachment now exists on the ext4 read-write CLI mount path; current crash/replay evidence is still required. |
...
| **Storage** | `ffs-block`, `ffs-journal`, `ffs-mvcc` | Block I/O with ARC / S3-FIFO cache; JBD2 replay + ext4 fast-commit + external-journal pairing; native MVCC with version chains, sharded store, snapshot isolation, four same-block merge mechanisms behind semantic proof labels, three conflict policies, Zstd/Brotli version compression, WAL persistence + recovery |
...
`MergeProof` labels describe candidate merge evidence. Source inspection on 2026-09-08 found four implemented merge mechanisms: `AppendOnly`, range overlay, `BitmapOr`, and `BitmapDelta`. `IndependentKeys` / `NonOverlappingExtents` / `TimestampOnlyInode` share the range-overlay validator; `DisjointBlocks` / `Unsafe` do not attempt same-block conflict resolution. This counts implemented algorithms, not executed validation results or mounted speedups.
...
| `BitmapOr` | Concurrent set-only allocations of different bits | Validate monotonicity and disjoint new bits; combine `latest \| staged` |
| `BitmapDelta` | Concurrent allocation/free of different bits | Validate disjoint changed-bit sets; combine `latest ^ base ^ staged` |
...
Replay is idempotent. V2/V3 checksums are verified with the kernel's seed (raw CRC32C from `~0` over the journal UUID), and a live log that runs past the end of the journal continues at `s_first`, as the kernel wraps it. Non-contiguous ext4 journal extents are supported. Both are checked against the kernel itself: `jbd2_wrapped_log_crash_image_recovery_matches_kernel` takes a crash image from a kernel mount whose live log wraps (start 921, end 490 of 1024 blocks), lets the kernel recover one copy, and requires FrankenFS's replay of another to produce the same 700 files byte for byte. Until 2026-09-26 the seed was wrong (raw CRC32C from 0), so FrankenFS replayed no transaction of any checksummed kernel journal, and a wrapped log lost every transaction after the wrap (bd-9m84h).
...
`SimulateOverlay` recovers into a private overlay and leaves the base image unchanged; `Skip` is diagnostic and does not provide a recovered view. The implementation covers bounded inline-extent and coordinated external-extent paths, not full ext4 fast-commit compatibility. The fast-commit area is read where the kernel writes it (the last `num_fc_blocks - 1` journal blocks), a cycle's commits after the first are read without a HEAD, each commit's TAIL tid and CRC are checked, and only the cycle that follows the log's last full commit is replayed — so a stale area left by a clean unmount or a full commit is not. Files the fast commits created are written from their INODE records and their inode and blocks are marked in the bitmaps and counters. A name removed as a file's last link (a DEL_ENTRY with no later INODE record) drops the link and frees the inode and its blocks, as the kernel's replay does. Certified against the kernel in four crash-image scenarios, each made durable only by `fsync` after a synced baseline (`fuse_e2e.rs::fast_commit_crash_image_*_bd_9m84h`): files created and appended; a hard link plus a last-link removal; a truncate plus an in-place overwrite; and a fast commit that a later full commit subsumed (the area must then be left unreplayed). In each, FrankenFS's recovery equals the kernel's own recovery of the same crash image — names, bytes and link counts — and `e2fsck -fn` is clean afterwards (bd-9m84h).
...
A **kernel**-written log (its log root tree: one ROOT_ITEM per subvolume pointing at that subvolume's log) is followed to the mounted subvolume's log and overlaid as a read view. A kernel log names a file created and fsynced since the last commit only through its INODE_REF items, so replay synthesizes the DIR_INDEX and DIR_ITEM entries those imply, as the kernel's `add_inode_ref` does; before 2026-09-26 such files were replayed as inodes with no name. Removals are read from the directory log ranges (DIR_LOG_INDEX): an FS-tree entry inside a logged range that the log does not carry was removed, as the kernel's `replay_dir_deletes` decides, and is hidden from the view. Two kernel-oracle tests check the result: a crash image made durable only through the tree log — new files and an append (`btrfs_kernel_tree_log_crash_image_matches_kernel`), or a removal plus a new file (`btrfs_kernel_tree_log_removal_matches_kernel`) — is recovered by a kernel mount and must read the same in FrankenFS. Not covered: extended refs (INODE_EXTREF) and renames across directories. Writes are refused while an unreplayable log is present.
...
| **`rename` overwrite atomicity** | On ext4, `rename(src, dst)` currently writes each touched block as its own MVCC commit, so a concurrent reader can observe an intermediate state. On disk it is atomic on a journalled `--rw` mount: a durability boundary (fsync, the periodic commit, a journal-pressure commit) waits for in-flight mutations before capturing, so it never persists half of one. Before that gate, a boundary racing renames made a half-rename durable (measured: e2fsck "ref count is 1, should be 2"). An unjournalled image has no crash atomicity at all. Tracked as bd-9rutw. | POSIX requires atomic replacement; the in-memory gap is a known deviation. |
