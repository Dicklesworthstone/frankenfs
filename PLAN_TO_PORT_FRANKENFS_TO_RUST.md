...
| **btrfs receive-side application + production-hardened multi-device RW** | V1 still requires multi-device reads; mounted device-set routing remains unfinished (`bd-hk5w3`). Chunk mapping/read helpers, transparent decompression, subvolume mount, tree-log replay, and send-stream parsing exist. `OpenFs::enable_writes` refuses multiple devices and profiles other than Single/DUP before mutable allocation setup, including when read validation is skipped. Receive-stream application and multi-device mutation remain later phases. |
